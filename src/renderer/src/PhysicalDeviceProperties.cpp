...
void PhysicalDeviceProperties::findQueueFamilies()
{
	...
	vkGetPhysicalDeviceQueueFamilyProperties(*pPhysicalDevice_, &queueFamilyCount, queueProperties.data());
	...
	for (const auto& queueProperty : queueProperties)
	{
		...
		vkGetPhysicalDeviceSurfaceSupportKHR(*pPhysicalDevice_, i, *pSurface_, &presentSupport);
...
void PhysicalDeviceProperties::querySwapChainSupport()
{
	...
	vkGetPhysicalDeviceSurfaceCapabilitiesKHR(*pPhysicalDevice_, *pSurface_, &swapChainDetails_.surfaceCapabilities);
	...
	if (surfaceFormatCount != 0)
	{
		...
		vkGetPhysicalDeviceSurfaceFormatsKHR(*pPhysicalDevice_, *pSurface_, &surfaceFormatCount, swapChainDetails_.surfaceFormats.data());
	}

	uint32_t presentModeCount = 0;
	vkGetPhysicalDeviceSurfacePresentModesKHR(*pPhysicalDevice_, *pSurface_, &presentModeCount, nullptr);

	if (presentModeCount != 0)
	{
		swapChainDetails_.presentModes.resize(presentModeCount);
		vkGetPhysicalDeviceSurfacePresentModesKHR(*pPhysicalDevice_, *pSurface_, &presentModeCount, swapChainDetails_.presentModes.data());
	}

}


bool PhysicalDeviceProperties::checkDeviceExtensionSupport(const std::vector<const char*>& extensions)const
{
	...
	vkEnumerateDeviceExtensionProperties(*pPhysicalDevice_, nullptr, &extensionCount, extensionsProperties.data());
...
VkFormat PhysicalDeviceProperties::findSupportedTilingFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features)const
{
...
PhysicalDeviceProperties::PhysicalDeviceProperties(const VkPhysicalDevice *pDevice, const VkSurfaceKHR* pSurface):
...
const VkPhysicalDeviceMemoryProperties & PhysicalDeviceProperties::getVkPhysicalDeviceMemoryProperties() const
{
