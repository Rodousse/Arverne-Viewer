...
void Model::assignMesh(const std::vector<Mesh> &meshes)
{
...
void Model::destroyMeshData(MeshData &meshData)
{
    ...
    vkDestroyBuffer(pCore_->getDevice(), meshData.vertexIndexBuffer, nullptr);
    vkFreeMemory(pCore_->getDevice(), meshData.vertexIndexBufferMemory, nullptr);
    meshData.isAllocated = false;
}

//------------------------------------------------------------------------------------------------------

void Model::setMaterialForMesh(const Mesh &mesh, const Material &material)
{
    auto itFound = std::find_if(meshes_.begin(), meshes_.end(), [&](Mesh other){
...
void Model::setMaterialForMeshData(MeshData &meshData, const Material &material)
{
...
void Model::createMeshData(const Mesh &mesh, MeshData &meshData)
{
...
void Model::createVertexBuffer(const Mesh& mesh, MeshData& meshData)
{
    ...
    pCore_->getUtils().createBuffer(bufferSize, usage, properties, stagingBuffer, stagingBufferMemory);
    ...
    vkMapMemory(pCore_->getDevice(), stagingBufferMemory, 0, bufferSize, 0, &pData);
    memcpy(pData, meshes_[0].vertices.data(), (size_t)bufferSize);
    vkUnmapMemory(pCore_->getDevice(), stagingBufferMemory);

    pCore_->getUtils().createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, meshData.vertexBuffer, meshData.vertexBufferMemory);
    pCore_->getUtils().copyBuffer(stagingBuffer, meshData.vertexBuffer, bufferSize);
...
void Model::createVertexIndexBuffer(const Mesh& mesh, MeshData& meshData)
{
    ...
    pCore_->getUtils().createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, meshData.vertexIndexBuffer, meshData.vertexIndexBufferMemory);
    pCore_->getUtils().copyBuffer(stagingBuffer, meshData.vertexIndexBuffer, bufferSize);
