...
bool PhysicalDeviceProvider::isDeviceContainingFeatures(VkPhysicalDeviceFeatures features) const
{
	for (size_t i = 0; i < sizeof(VkPhysicalDeviceFeatures) / sizeof(VkBool32); i++)
	{
		VkBool32* currentFeature = (reinterpret_cast<VkBool32*>(&features)) + (i * sizeof(VkBool32));
		const VkBool32* currentRequiredFeature = (reinterpret_cast<const VkBool32*>(&requiredFeatures_)) + (i * sizeof(VkBool32));
...
PhysicalDeviceProvider::PhysicalDeviceProvider(const VulkanCore * pCore, const std::vector<const char*> neededExtensions):
	pCore_(pCore)
{
...
void PhysicalDeviceProvider::setNeededDeviceExtensions(const std::vector<const char*> neededExtensions)
{
